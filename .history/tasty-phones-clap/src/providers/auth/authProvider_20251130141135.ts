...
export const authProvider = {
  ...
  login: async ({ email, password }: ILoginForm) => {
    try {
      ...
      // 🚨 QUAN TRỌNG: Đảm bảo lưu đúng structure
      console.log("💾 Saving auth to localStorage:", authState);
      localStorage.setItem("auth", JSON.stringify(authState));

      // 🚨 QUAN TRỌNG: Đọc lại từ localStorage để verify
      const verifyAuth = localStorage.getItem("auth");
      console.log("✅ Verified auth in localStorage:", verifyAuth);

      const role = authState.role || apiData.user.role;
      ...
      return {
        success: true,
        redirectTo,
        // 🚨 THÊM user data để component có thể sử dụng ngay
        user: authState,
      };
    } catch (error: any) {
