...
import { List, useTable, DateField, getDefaultFilter } from "@refinedev/antd";
...
export const UserList: React.FC = () => {
  ...
  // Sử dụng useTable đơn giản hơn
  const { tableProps, tableQueryResult, filters, setFilters } = useTable<User>({
    resource: "users",
    pagination: {
      mode: "server",
      pageSize: 20,
    },
    syncWithLocation: true, // Quan trọng: giữ filter trong URL
  });
  ...
  // Khởi tạo form với giá trị từ filters
  useEffect(() => {
    const nameFilter = getDefaultFilter("user_name", filters);
    const roleFilter = getDefaultFilter("role", filters);

    form.setFieldsValue({
      user_name: nameFilter?.value || "",
  ...
  // Lấy giá trị filter hiện tại để hiển thị
  const currentNameFilter = getDefaultFilter("user_name", filters);
  const currentRoleFilter = getDefaultFilter("role", filters);
  const searchName = currentNameFilter?.value || "";
  const roleFilter = currentRoleFilter?.value || null;
        ...
        >
          <Row gutter={[16, 16]}>
            <Col xs={24} sm={12} md={10}>
              <Form.Item label="Tìm theo tên tài khoản" name="user_name">
                <Input
          ...
          <Space direction="vertical" style={{ width: "100%" }} size="large">
            <div>
              ...
              <div style={{ marginTop: 4 }}>
                <Text strong>{selectedUser?.user_name || "Chưa đặt tên"}</Text>
              </div>
            </div>
