...
export default function BookingShow() {
  ...
  const handleAddPenalties = async () => {
    ...
    try {
      ...
      // Gọi API đúng theo cấu trúc bạn đã test
      const response = await axios.post(
        `${API_URL}/api/bookings/${id}/penalties`, // API không phải admin
        {
  ...
  const canAddPenalties = ["checked_in", "check_out", "completed"].includes(
    booking.status
  );
