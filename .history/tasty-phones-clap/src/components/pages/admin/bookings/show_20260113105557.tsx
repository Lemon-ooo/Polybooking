  ...
  const fetchAvailableRooms = async () => {
    ...
    try {
      ...
      for (const endpoint of endpoints) {
        try {
          ...
          if (response.data.success && response.data.data) {
            ...
            rooms = response.data.filter((room: any) => room.room_type_id === roomTypeId);
            break;
          } else if (response.data.rooms) {
            rooms = response.data.rooms.filter((room: any) => room.room_type_id === roomTypeId);
            break;
          }
        } catch (error) {
          console.log(`Endpoint ${endpoint} failed`);
      ...
      if (rooms.length === 0) {
        ...
        console.log(`Found ${rooms.length} available rooms of type ${roomTypeName}`);
      }
    } catch (error: any) {
      ...
      <Row gutter={[24, 24]}>
        ...
        <Col xs={24} lg={16}>
          ...
          {hasAssignedRooms && (
            ...
            >
              <Alert
                message={`Đã gắn ${assignedRooms.length} phòng ${roomTypeName} cho booking này`}
                type="success"
                showIcon
                style={{ marginBottom: 16 }}
              />
            ...
            >
              <div style={{ textAlign: "center", padding: "20px" }}>
                ...
                <div style={{ color: "#666", marginBottom: "16px" }}>
                  Booking này chưa được gắn phòng. Vui lòng gắn phòng {roomTypeName} để khách
                  có thể check-in.
                </div>
                <Button
                  type="primary"
          ...
          <Alert
            ...
            description={
              <div>
                <Row gutter={16}>
                  <Col span={12}>
                    <div><strong>Loại phòng đặt:</strong></div>
          ...
          {roomSearchLoading ? (
            ...
            <div style={{ textAlign: "center", padding: "40px 0" }}>
              ...
              <div style={{ fontSize: "14px", color: "#666" }}>
                ...
                gian {formatDate(booking?.check_in || "")} - {formatDate(booking?.check_out || "")}.
              </div>
            </div>
          )}
