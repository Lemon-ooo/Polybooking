...
import { useLogin, useRouterContext } from "@refinedev/core"; // ✅ Thêm useRouterContext
...
export const Login: React.FC = () => {
  ...
  const { replace } = useRouterContext(); // ✅ Dùng useRouterContext để redirect
  ...
  const onFinish = async (values: any) => {
    ...
    login(values, {
      onSuccess: (data) => {
        // ✅ Redirect sẽ được xử lý trong authProvider, không cần làm gì thêm
        if (data?.success) {
          // AuthProvider đã xử lý redirect
        }
      },
      onError: (error: any) => {
