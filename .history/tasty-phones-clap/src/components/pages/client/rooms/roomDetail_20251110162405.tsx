...
export const RoomDetail: React.FC = () => {
  ...
  // Sử dụng useTable giống như RoomList
  const { tableQueryResult } = useTable<Room>({
    resource: "rooms",
  });
  ...
  const room = useMemo(() => {
    ...
    const rooms = Array.isArray(data.data) ? data.data : data.data.data || [];
    return rooms.find((room: Room) => room.id.toString() === id);
  }, [data, id]);

  // Debug
  console.log("=== ROOM DETAIL SIMPLE ===");
  console.log("Room ID:", id);
  console.log("All rooms data:", data?.data);
