...
export const ClientRooms: React.FC = () => {
  ...
  // Function to get category title for a room
  const getCategoryTitle = (roomName: string | undefined) => {
    if (!roomName) return null;
    if (roomName.includes("Premier")) return "Premier";
    if (roomName.includes("Deluxe")) return "Deluxe";
    return null;
  };

  // Function to get the room sub-name (without category prefix)
  const getRoomSubName = (roomName: string | undefined) => {
    if (!roomName) return roomName;
    const prefix = getCategoryTitle(roomName);
    if (prefix) {
      return roomName.replace(prefix, "").trim();
    }
    return roomName;
  };
            ...
            <>
              <Row gutter={[0, 48]} justify="center">
                {rooms.map((room, index) => {
                  ...
                  const categoryTitle = getCategoryTitle(roomName);
                  const roomSubName = getRoomSubName(roomName);
                  const showCategory =
                    categoryTitle &&
                    index ===
                      rooms.findIndex(
                        (r) =>
                          getCategoryTitle(
                            r.room_type?.name || `Room ${r.room_number}`
                          ) === categoryTitle
                      );
                      ...
                      >
                        {showCategory && (
                          <div className="category-section-title">
                            <Title
                              level={3}
                              className="category-title luxury-category-title"
                            >
                              {categoryTitle}
                            </Title>
                            <Divider
                        ...
                        <Row gutter={[32, 32]} align="top">
                          ...
                          <Col xs={24} md={14}>
                            <div className="room-info luxury-room-info">
                              ...
                              >
                                {showCategory ? roomSubName : roomName}
                              </Title>
