...
export const RoomDetail: React.FC = () => {
  ...
  // Sử dụng useTable với error handling
  const tableResult = useTable<Room>({
    resource: "rooms",
  });

  // Safe destructuring với fallback values
  const data = tableResult?.tableQueryResult?.data;
  const isLoading = tableResult?.tableQueryResult?.isLoading ?? false;
  const isError = tableResult?.tableQueryResult?.isError ?? false;
  const error = tableResult?.tableQueryResult?.error;

  // Tìm phòng theo ID từ danh sách
  const room = useMemo(() => {
    if (!data) return null;

    // Xử lý nhiều định dạng data khác nhau
    let rooms: Room[] = [];
  ...
  const roomName = room?.room_type?.name || `Phòng ${room?.room_number || id}`;
  const roomDescription =
    room?.description ||
    room?.room_type?.description ||
    "Phòng nghỉ sang trọng với đầy đủ tiện nghi hiện đại.";
  const roomPrice = room?.price || "2000000";
  const roomStatus = room?.status || "available";

  const handleBookNow = () => {
    if (bookingDates && room) {
    ...
    return (
      <div className="error-container">
        ...
        <div style={{ marginTop: 16, textAlign: "center" }}>
          <Button type="primary" onClick={() => navigate("/client/rooms")}>
          ...
          <Col xs={24} lg={16}>
            ...
            <div className="image-gallery">
              <div className="main-image">
                ...
                />
                <Tag
                  color={getRoomStatusColor(roomStatus)}
          ...
          <Col xs={24} lg={8}>
            <Card className="booking-card" bordered={false}>
              <div className="price-section">
                ...
                <Text className="price-amount">{formatPrice(roomPrice)}</Text>
